...
// Função para inicializar produtos com imagens em base64
async function initializeProductsWithBase64Images() {
  // Cria uma cópia profunda dos produtos
  const productsWithBase64 = JSON.parse(JSON.stringify(products));
  
  // Converte todas as imagens para base64
  for (let i = 0; i < productsWithBase64.length; i++) {
    if (productsWithBase64[i].image) {
      productsWithBase64[i].image = await urlToBase64(productsWithBase64[i].image);
    }
  }
...
export { initializeProductsWithBase64Images };
